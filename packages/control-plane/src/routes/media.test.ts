...
describe("media routes", () => {
  describe("POST /sessions/:id/media/upload", () => {
    it("uploads a file and returns key + url", async () => {
      const env = createMockEnv();
      const route = findRoute("POST", "/sessions/sess-123/media/upload")!;
      expect(route).toBeDefined();

      const body = new ArrayBuffer(100);
      const request = new Request("https://test.local/sessions/sess-123/media/upload", {
        method: "POST",
        headers: {
      ...
      const match = "/sessions/sess-123/media/upload".match(route.pattern)!;
    ...
    it("rejects SVG uploads", async () => {
      ...
      const request = new Request("https://test.local/sessions/sess-123/media/upload", {
        method: "POST",
        headers: { "content-type": "image/svg+xml", "x-filename": "test.svg" },
  ...
  describe("GET /api/media/:key", () => {
    it("returns the stored object with correct content type and security headers", async () => {
      const env = createMockEnv();
      const uploadRoute = findRoute("POST", "/sessions/sess-123/media/upload")!;
      const downloadRoute = findRoute("GET", "/api/media/test-key")!;
      expect(downloadRoute).toBeDefined();

      // Upload first
      const uploadReq = new Request("https://test.local/sessions/sess-123/media/upload", {
        method: "POST",
        headers: { "content-type": "image/png", "x-filename": "test.png" },
        body: new ArrayBuffer(64),
      });
      const uploadMatch = "/sessions/sess-123/media/upload".match(uploadRoute.pattern)!;
      const uploadResp = await uploadRoute.handler(uploadReq, env, uploadMatch, createCtx());
      const { key } = await uploadResp.json<{ key: string }>();

      // Download (key must be URL-encoded since it contains session prefix with /)
      const encodedKey = encodeURIComponent(key);
      const downloadReq = new Request(`https://test.local/api/media/${encodedKey}`);
      const downloadMatch = `/api/media/${encodedKey}`.match(downloadRoute.pattern)!;
