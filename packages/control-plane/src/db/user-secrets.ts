...
const log = createLogger("user-secrets");

export class UserSecretsStore {
  constructor(
  ...
  async setSecrets(
    ...
    if (existingKeySet.size + netNew > MAX_SECRETS_PER_SCOPE) {
      throw new SecretsValidationError(
        `User would exceed ${MAX_SECRETS_PER_SCOPE} secrets limit ` +
        ...
        this.db
          .prepare(
            `INSERT INTO user_secrets
             (user_id, key, encrypted_value, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?)
             ON CONFLICT(user_id, key) DO UPDATE SET
               encrypted_value = excluded.encrypted_value,
               updated_at = excluded.updated_at`
          )
          .bind(userId, key, encrypted, now, now)
      );
    }
  ...
  async listSecretKeys(userId: string): Promise<SecretMetadata[]> {
    const result = await this.db
      .prepare(
        "SELECT key, created_at, updated_at FROM user_secrets WHERE user_id = ? ORDER BY key"
      )
      .bind(userId)
  ...
  async getDecryptedSecrets(userId: string): Promise<Record<string, string>> {
    const result = await this.db
      .prepare("SELECT key, encrypted_value FROM user_secrets WHERE user_id = ?")
      .bind(userId)
