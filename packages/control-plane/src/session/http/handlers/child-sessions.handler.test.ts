...
function createSessionRepo(overrides: Partial<SessionRepoRow> = {}): SessionRepoRow {
  return {
    id: "session-repo-1",
    repo_owner: "acme",
    repo_name: "repo",
...
function createHandler() {
  ...
  const mapRepoRowToScope = vi.fn<(repo: SessionRepoRow) => SessionRepoScope>();
...
describe("createChildSessionsHandler", () => {
  ...
  it("maps spawn context from session and owner participant", async () => {
    const { handler, getSession, repository, mapRepoRowToScope } = createHandler();
    ...
    repository.listSessionRepos.mockReturnValue([createSessionRepo()]);
    mapRepoRowToScope.mockReturnValue({
      repoOwner: "acme",
      repoName: "repo",
      repoId: 123,
      order: 0,
