...
import type { SandboxRow, SessionRepoRow, SessionRow } from "../../types";

export interface ChildSessionsHandlerDeps {
  repository: Pick<
    SessionRepository,
    "listParticipants" | "listArtifacts" | "listEvents" | "listSessionRepos"
  >;
  getSession: () => SessionRow | null;
  getSandbox: () => SandboxRow | null;
  getPublicSessionId: (session: SessionRow) => string;
  mapRepoRowToScope: (repo: SessionRepoRow) => SessionRepoScope;
  broadcast: (message: {
...
export function createChildSessionsHandler(deps: ChildSessionsHandlerDeps): ChildSessionsHandler {
  return {
    getSpawnContext(): Response {
      ...
      const sessionRepos = deps.repository
        .listSessionRepos()
        .map((repo) => deps.mapRepoRowToScope(repo));
